...
- 單身視窗新增欄位離開試算連動（來源單號/製程/數量/單價），可預先計算 `PURM2/PURP2` 並即時提示錯誤。
- `OdbcPurchaseOrderService` 報表輸出改為 `#ATEMP` 暫存表流程，分成 Query12(抬頭摘要) / Query13(明細) 文字輸出。
- 報表文字輸出改為固定欄寬表格格式，新增欄位標題、分隔線與總筆數資訊。
