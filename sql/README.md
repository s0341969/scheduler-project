# 專案說明
## 2026-05-21 新增 ML/APS 導入評估文件

- 新增評估文件：[排程系統導入 MLAPS 的落地藍圖.md](G:\codex_pg\sql\排程系統導入 MLAPS 的落地藍圖.md)。
- 本文件定位為「評估版」，只討論既有排程系統導入 `ML` / `APS` 的資料需求、分階段路線與風險。
- 不修改目前 SQL 正式執行流程。
- 不代表立即導入模型或改寫 `dbo.產生ORDE3剩餘製程`。
...
## 2026-04-02 第六輪實驗（已回退）

- 嘗試內容：
  - 將 HM 指派流程再整併為「單一序號表 + 單次回寫」。
- `%` 測試結果（3 次）：
  - `341,229 ms`、`377,418 ms`、`355,876 ms`
  - 平均 `358,174 ms`
- 結論：
