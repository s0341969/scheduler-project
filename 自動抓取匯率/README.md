...
5. 連線字串若未在畫面填寫，會自動 fallback 讀取環境變數 `BOT_RATE_DB_CONN`。
6. Timer 觸發時若上一輪尚未完成，會自動略過，避免重疊執行。
7. MSSQL 寫入採唯一索引去重，鍵值為 `SourceUpdatedAt + CurrencyCode`，可避免同一掛牌時間重複寫入。
