...
8. 畫面可設定：
   ...
   - 抓取頻率單位（分鐘 / 小時）
   - HTTP 逾時秒數
9. 兩個寫入選項預設都未勾選。未勾選時，程式只抓取資料並顯示結果，不會連線或寫入 MSSQL。
10. 每次抓取成功後，畫面下方的 `DataGridView` 會顯示最新匯率資料，包含幣別、現金本行買入、現金本行賣出、掛牌日期與更新時間。
11. 連線字串若未在畫面填寫，會自動 fallback 讀取環境變數 `BOT_RATE_DB_CONN`。
12. Timer 觸發時若上一輪尚未完成，會自動略過，避免重疊執行。
