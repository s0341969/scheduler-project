...
5. 寫入 `FTIL / FTOL` 的規則依你提供的舊邏輯：
   - `CNY`：使用現金買入 / 現金賣出
   - 其他幣別：使用即期買入 / 即期賣出
6. 寫入資料庫時，改為只有匯率真的變動才寫：
   - `CHRNAME`：只有 `FTIL / FTOL` 任一變動才更新；若該幣別不存在則新增
   - `CHRNAME-HISTORY`：只有和該幣別最新歷史匯率不同時才新增；若該幣別尚無歷史則新增第一筆
7. `CHRNAM` 依你提供的 legacy 規則轉換：
   - `USD -> US`
   - `JPY -> JP`
   - `THB -> TA`
   - `CNY -> RMB`
   ...
   - `SGD -> SGD`
   - `AUD -> AUD`
8. 寫入 `CHRNAME` 與 `CHRNAME-HISTORY` 時，會自動偵測說明欄位：
   - 若存在 `CHRDS`，寫入 `CHRDS`
...
9. 畫面可設定：
   ...
   - 抓取頻率單位（分鐘 / 小時）
   - HTTP 逾時秒數
10. 兩個寫入選項預設都未勾選。未勾選時，程式只抓取資料並顯示結果，不會連線或寫入 MSSQL。
11. 每次抓取成功後，畫面下方的 `DataGridView` 會顯示最新匯率資料，包含幣別、現金本行買入、現金本行賣出、掛牌日期與更新時間。
12. 連線字串若未在畫面填寫，會自動 fallback 讀取環境變數 `BOT_RATE_DB_CONN`。
13. Timer 觸發時若上一輪尚未完成，會自動略過，避免重疊執行。
14. 所有 `.cs` 檔已補上繁體中文功能註解，包含類別、主要方法與核心屬性，方便在 VS2015 中直接閱讀維護。
15. 主要 UI 訊息、錯誤訊息與程式常用文字已改為直接使用繁體中文字串，不再保留 `\uXXXX` 跳脫格式。
